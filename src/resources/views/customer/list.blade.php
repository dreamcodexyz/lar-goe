                        ...
                        <table class="table table-hover demo-table-dynamic table-responsive-block dataTable no-footer" id="tableWithDynamicRows" role="grid" aria-describedby="tableWithDynamicRows_info">
                            <thead>
                                <tr role="row">
                                    ...
                                    <th class="sorting" tabindex="0" aria-controls="tableWithDynamicRows" rowspan="1" colspan="1" aria-label="Description: activate to sort column ascending" style="width: 100px;">Ngày sinh</th>
                                    <th class="sorting" tabindex="0" aria-controls="tableWithDynamicRows" rowspan="1" colspan="1" aria-label="Price: activate to sort column ascending" style="width: 100px;">Giới tính</th>
                                    {{--<th class="sorting" tabindex="0" aria-controls="tableWithDynamicRows" rowspan="1" colspan="1" aria-label="Notes: activate to sort column ascending" style="width: 100px;">Lớp</th>--}}
                            ...
                            <tbody>
                                ...
                                @foreach ($list_data as $customer)

                                    <tr role="row" class="{{ $customer->id % 2 == 0 ? 'odd' : 'event' }}">
                                        <td class="v-align-middle sorting_1">
                                            <input type="checkbox" class="cbox_customer_id" name="customers[]" value="{!! $customer->id !!}">
                                        </td>
                                        ...
                                        <td class="v-align-middle">
                                            <p>{!! date('d-m-Y', strtotime($customer->birthday)) !!}</p>
                                        </td>
                                        <td class="v-align-middle">
                                            <p>
                                            ...
                                            <p>
                                                @if (isset($store_options))
                                                    @foreach ($store_options as $val)
                                                        {{ $customer->store_id == $val['value'] ? $val['label'] : '' }}
                                                    @endforeach
                                                @endif
                                            </p>
                                        </td>
                                            ...
                                            <p>
                                                @if (isset($status_options))
                                                    @foreach ($status_options as $status)
                                                        {{ $customer->status == $status['value'] ? $status['label'] : '' }}
                                                    @endforeach
                                                @endif
                                            </p>
                                        </td>
