        ...
        <div class="panel panel-transparent">
            <div class="panel-heading">
                ...
                <div class="pull-right m-b-10">
                    <div class="col-xs-12">
                        <a href="{{ url('parent/new') }}" class="btn btn-default "><i class="fa pg-plus"></i>  {{ __('common.add_new') }}</a>
                        ...
                        <table class="table table-hover demo-table-dynamic table-responsive-block dataTable no-footer" id="tableWithDynamicRows" role="grid" aria-describedby="tableWithDynamicRows_info">
                            <thead>
                                <tr role="row">
                                    ...
                                    <th style="width:100px" class="sorting_asc" tabindex="0" aria-controls="tableWithDynamicRows" rowspan="1" colspan="1" aria-label="App name: activate to sort column descending" aria-sort="ascending">Điện thoại</th>
                                    <th style="width:100px" class="sorting" tabindex="0" aria-controls="tableWithDynamicRows" rowspan="1" colspan="1" aria-label="Price: activate to sort column ascending" >Giới tính</th>
                                    <th class="sorting" tabindex="0" aria-controls="tableWithDynamicRows" rowspan="1" colspan="1" aria-label="Notes: activate to sort column ascending" >Phụ huynh của</th>
                                    <th class="sorting" tabindex="0" aria-controls="tableWithDynamicRows" rowspan="1" colspan="1" aria-label="Notes: activate to sort column ascending" >Ghi chú</th>
                            ...
                            <tbody>
                                ...
                                @foreach ($list_data as $data)
                                    ...
                                    <tr role="row" class="{{ $data->id % 2 == 0 ? 'odd' : 'event' }}">
                                        ...
                                        <td class="v-align-middle">
                                            <p class="text-center"> {{ $data->gender == 2 ? __("customer.form_attribute.gender_f") : __("customer.form_attribute.gender_m") }}</p>
                                        </td>

                                        <td class="v-align-middle">
                                            <p>
                                                @if($data->customer()->value('id'))
                                                <a href="{{ url('customer/edit/'.$data->customer()->value('id')) }}">{!! $data->customer()->value('name') !!}</a>
                                                @else
                                                    Chưa cập nhật
                                                @endif
                                            </p>
                                        ...
                                        <td class="v-align-middle">
                                            <p class="text-center"><a href="{{ url('parent/edit/'.$data->id) }}">{{ __('common.edit') }}</a> | <a href="{{ url('parent/delete/'.$data->id) }}">{{ __('common.delete') }}</a></p>
                                        </td>
                                    </tr>
